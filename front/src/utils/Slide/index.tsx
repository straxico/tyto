...
export const StyledSlide = styled.div<SlideProps & SlideState>`
  position: relative;
  width: 100%;
  transition: max-height ${({ theme }) => theme.jajiga.durationFast} linear;
...
class Slide extends React.Component<SlideProps, SlideState> {
  ...
  componentDidUpdate(prevProps: SlideProps, prevState: SlideState, snapshot: null | true) {
    if (snapshot) {
      if (this.props.expanded) {
        this.setMaxHeight();
  ...
  getSnapshotBeforeUpdate(prevProps: SlideProps) {
