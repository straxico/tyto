...
export const StyledLoading = styled(({ children, className, dataTest }) => (
  ...
  ${getHeight};
  padding: ${({ theme, type }) =>
    type !== TYPE_OPTIONS.INLINE_LOADER && theme.jajiga.paddingLoading};
...
const StyledLoadingText = styled.div<{ type: LoadingType }>`
  font-family: ${({ theme }) => theme.jajiga.fontFamily};
  font-size: ${({ theme }) => theme.jajiga.fontSizeTextNormal};
  color: ${({ theme }) => theme.jajiga.colorTextLoading};
  line-height: ${({ theme }) => theme.jajiga.lineHeightText};
  margin-top: ${({ theme, type }) => type === TYPE_OPTIONS.PAGE_LOADER && theme.jajiga.spaceMedium};
  margin-${left}: ${({ theme, type }) =>
  type !== TYPE_OPTIONS.PAGE_LOADER && theme.jajiga.spaceSmall};
`;
...
const StyledSpinnerCircle = styled.circle`
  fill: transparent;
  stroke: ${({ theme, type }) =>
    type === TYPE_OPTIONS.BUTTON_LOADER ? 'currentColor' : theme.jajiga.paletteInkLighter};
  stroke-width: 3px;
...
const StyledLoaderCircle = styled.div`
  ...
  border-radius: 50%;
  margin-${right}: 6px;
  background: ${({ theme }) => theme.jajiga.paletteInkLighter};
