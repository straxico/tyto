...
describe(`Textarea with help`, () => {
  const size = SIZE_OPTIONS.NORMAL as TextAreaSize;
  const name = 'name';
  const label = 'Label';
  const value = 'value';
  ...
  it('should have passed props', () => {
    ...
    expect(area.render().prop('tabindex')).toBe('-1');
    expect(
      component
...
describe(`Textarea number with error and help`, () => {
  const size = SIZE_OPTIONS.SMALL as TextAreaSize;
  const resize = RESIZE_OPTIONS.NONE as TextAreaResize;
