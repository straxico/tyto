...
const getBorder = ({ theme }) =>
  `${theme.jajiga.borderWidthCard} ${theme.jajiga.borderStyleCard} ${theme.jajiga.borderColorCard}`;

const getBorderRadius = ({ theme }) => theme.jajiga.borderRadiusNormal;
...
const StyledChildWrapper = styled.div<{
  ...
  margin: ${({ theme, expanded }) => expanded && `${theme.jajiga.spaceXSmall} 0`};
  transition: margin ${({ theme, initialExpanded }) =>
    !initialExpanded && theme.jajiga.durationFast} ease-in-out;
...
const StyledCard = styled.div<{ closable; hasAdjustedHeader; spaceAfter }>`
  width: 100%;
  ...
  ${StyledCardHeader} {
    padding-${right}: ${({ theme, closable }) => closable && theme.jajiga.spaceLarge};
