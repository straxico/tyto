...
const getSizeToken = () => ({ size, theme }) => {
  const tokens = {
    ...
    [SIZES.SMALL]: theme.jajiga.widthModalSmall,
    [SIZES.NORMAL]: theme.jajiga.widthModalNormal,
    [SIZES.LARGE]: theme.jajiga.widthModalLarge,
  };
...
const ModalBody = styled.div<{ onScroll; onClick }>`
  width: 100%;
  height: 100%;
  position: fixed;
  top: 0;
  ...
  bottom: 0;
  left: 0;
  z-index: ${({ theme }) => theme.jajiga.zIndexModalOverlay};
...
const ModalWrapper = styled.div<{ isMobileFullPage; loaded; onScroll; size; fixedFooter }>`
  box-sizing: border-box;
  min-height: 100%;
  display: flex;
...
const CloseContainer = styled.div<{ scrolled; fixedClose; isMobileFullPage; modalWidth; size }>`
  display: flex;
  ...
  background-color: ${({ theme, scrolled }) => scrolled && theme.jajiga.paletteWhite};
  border-top-left-radius: ${({ isMobileFullPage }) =>
    !isMobileFullPage && '9px'}; // TODO: create token
  border-top-right-radius: ${({ isMobileFullPage }) =>
    !isMobileFullPage && '9px'}; // TODO: create token
  ...
  ${media.largeMobile(css<{ scrolled; fixedClose }>`
  ...
  ${StyledButtonLink} {
    margin-${right}: ${({ theme }) => theme.jajiga.spaceXXSmall};
    ...
    &:active svg {
      color: ${({ theme }) => theme.jajiga.paletteInkLightActive};
    }
  }
`;
...
const ModalWrapperContent = styled.div<{
  ...
  border-top-right-radius: ${({ isMobileFullPage }) =>
    !isMobileFullPage && '9px'}; // TODO: create token
  background-color: ${({ theme }) => theme.jajiga.backgroundModal};
  font-family: ${({ theme }) => theme.jajiga.fontFamily};
  width: 100%;
  ${({ theme, fixedFooter, footerHeight, isMobileFullPage }) =>
    isMobileFullPage
      ...
      : css`
          max-height: calc(
            100% - ${theme.jajiga.spaceXLarge} -
              ${`${fixedFooter && !!footerHeight ? footerHeight : 0}px`}
          );
        `};
  bottom: ${({ fixedFooter, footerHeight, isMobileFullPage, theme }) =>
    `${(!isMobileFullPage ? parseInt(theme.jajiga.spaceXLarge, 10) : 0) +
      (fixedFooter && !!footerHeight ? footerHeight : 0)}px`};
  box-shadow: ${({ theme }) => theme.jajiga.boxShadowModal};
  overflow-y: auto;
  overflow-x: hidden;
  ...
  ${({ fixedFooter, theme, footerHeight, fullyScrolled }) =>
    ...
    css`
      ${StyledModalFooter} {
        bottom: 0;
        padding: ${theme.jajiga.spaceMedium};
        box-shadow: ${fullyScrolled
          ? `inset 0 1px 0 ${theme.jajiga.paletteCloudNormal}, 0 -2px 4px 0 rgba(23, 27, 30, 0)`
      ...
      ${StyledModalSection}:last-of-type {
        padding-bottom: ${theme.jajiga.spaceLarge};
        margin-bottom: 0;
      }
    `};
  ...
  ${MobileHeader} {
    top: ${({ scrolled, theme, isMobileFullPage }) =>
      !isMobileFullPage && scrolled && theme.jajiga.spaceXLarge};
    opacity: ${({ scrolled }) => scrolled && '1'};
    visibility: ${({ scrolled }) => scrolled && 'visible'};
    transition: ${({ scrolled, theme }) =>
      scrolled &&
      `top ${theme.jajiga.durationNormal} ease-in-out,
    opacity ${theme.jajiga.durationFast} ease-in-out,
    visibility ${theme.jajiga.durationFast} ease-in-out ${theme.jajiga.durationFast}`};
  }
  ...
  ${StyledModalHeader} {
    margin-bottom: ${({ hasModalSection, theme }) => !hasModalSection && theme.jajiga.spaceXLarge};
  }
  ...
  ${media.largeMobile(css<{
    ...
    max-height: 100%;
    ${StyledModalSection}:last-of-type {
      padding-bottom: ${({ theme }) => theme.jajiga.spaceXXLarge};
    ...
    ${StyledModalFooter} {
      padding: ${({ theme, fixedFooter }) =>
        fixedFooter
          ? `${theme.jajiga.spaceXLarge} ${theme.jajiga.spaceXXLarge}!important`
    ...
    ${MobileHeader} {
      top: ${({ scrolled, theme }) => (scrolled ? '0' : `-${theme.jajiga.spaceXXLarge}`)};
      width: ${({ modalWidth, theme }) =>
        `calc(${modalWidth}px - 48px - ${theme.jajiga.spaceXXLarge})`};
    }
  `)};
      ...
      css<{ fullyScrolled; fixedFooter }>`
        ${StyledModalFooter} {
          ...
          box-shadow: ${({ fixedFooter }) =>
            !fixedFooter && `inset 0 0 0 1px ${theme.jajiga.paletteWhite}`};
        }
        ...
        ${({ fullyScrolled, fixedFooter }) =>
          ...
          `};
      `,
      getBreakpointWidth(QUERIES.LARGEMOBILE, theme) as string,
    )};
`;
...
export class PureModal extends React.PureComponent<ModalProps, ModalState> {
  static defaultProps = {
    theme: defaultTheme,
  };
  ...
  setScrollPosition = (value: number) => {
    ...
    if (
      window === null
        ? 0
        : window.innerWidth >= getBreakpointWidth(QUERIES.LARGEMOBILE, this.props.theme, true)
    ) {
  ...
  setDimensions = () => {
    ...
    if (content) {
      ...
      this.offset =
        (headingEl ? headingEl.clientHeight : 0) + (headingEl ? headingEl.offsetTop : 0);
  ...
  decideFixedFooter = () => {
    ...
    const fullyScrolled =
      (content ? content.scrollHeight : 0) + 40 - (body ? body.scrollTop : window.innerHeight) >= 0;
...
const Modal = withTheme(PureModal);
