...
const StyledSliderContent = styled.div<{ focused }>`
  display: block;
  width: 100%;
  box-sizing: border-box;
  padding-bottom: ${({ theme }) => theme.jajiga.spaceXSmall};

  ${mq.tablet(css<{ focused }>`
    width: calc(100% + 48px);
    z-index: 10;
    position: absolute;
    bottom: -16px;
    ...
    ${({ focused, theme }) =>
      ...
      css`
        visibility: visible;
        opacity: 1;
        background: ${theme.jajiga.paletteWhite};
        box-shadow: 0 2px 4px 0 ${convertHexToRgba(theme.jajiga.paletteInkLight, 24)},
          0 4px 12px 0 ${convertHexToRgba(theme.jajiga.paletteInkLight, 32)};
      `};
  `)};
`;
...
export class PureSlider extends React.PureComponent<SliderProps, SliderState> {
  static defaultProps = {
    theme: defaultTheme,
  };
  ...
  isNotEqual = (a: SliderValue, b: SliderValue) => {
  ...
  sortArray = (arr: SliderValue): SliderValue => {
  ...
  findClosestKey = (goal: number, value: SliderValue) => {
  ...
  injectCallbackAndSetState = (
    callback: SliderCallback | null,
    newValue: SliderValue | null,
    forced: boolean | null = false,
  ) => {
    const { value } = this.state;
    if (newValue != null) {
  ...
  handleMouseDown = (i: number | null) => event => {
  ...
  handleOnTouchStart = (i: number | null) => event => {
  ...
  replaceValue = (newValue: number, index: number | null) => {
    const { value } = this.state;
  ...
  render() {
    ...
    if (histogramData) {
      ...
      console.log(properHistogramLength);
    }
